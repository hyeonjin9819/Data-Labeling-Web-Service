...
function RegisterPage() {
  ...
  const onAuthHandler = (event: { currentTarget: { value: React.SetStateAction<string>; }; }) => {
  ...
  return (
    <div className="form-container sign-up-container" style={{display : 'inline'}}>
    <form className="form" onSubmit={onSubmitHandler}>
        <h1 className="form-title">Hello, Friend!</h1>
        <input type="text" placeholder="이름" value = {Name} onChange={onNameHandler}/>
        <input type="email" placeholder="이메일" value = {Email} onChange={onEmailHandler} />
        <div>
        <button className = 'auth_email' onClick = {sendEmail}> 전송 </button>
        <p style = {{display:'inline'}}> 이메일 인증을 해주세요 </p>
        </div>
        <input type="text" placeholder="인증번호를 입력해주세요" value = {Auth} onChange={onAuthHandler} />
        <input type="password" placeholder="패스워드" value = {Password} onChange={onPasswordHandler} />
        <input type="password" placeholder='패스워드 확인' value={ConfirmPassword} onChange={onConfirmPasswordHandler} />

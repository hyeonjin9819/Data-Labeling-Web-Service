...
class S3UpDownLoader():
    ...
    def upload_file(self, src_path, dest_path):
        ...
        if self.verbose == True:
            ...
            time.sleep(0.3)
            with tqdm(total=file_size, unit='B', unit_scale=True, desc=file_name, ascii=True) as t:
                self.s3.meta.client.upload_file(src_path, self.bucket_name, destination_path, Config=self.config, Callback=self.get_bytes(t), ExtraArgs={'ACL': 'public-read'}) 
        else:
            print(">>> upload file : (Local) " + src_path + " -> (S3 Storage) " + destination_path)
            self.s3.meta.client.upload_file(src_path, self.bucket_name, destination_path, Config=self.config, ExtraArgs={'ACL': 'public-read'}) 
